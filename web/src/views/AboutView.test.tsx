...
test("renders methodology sections and the transform", () => {
  ...
  expect(screen.getByText(/log total fertility rate/i)).toBeInTheDocument(); // log transform surfaced
...
test("explains the per-factor transforms", () => {
  render(<AboutView bundle={bundle} />);
  expect(screen.getByRole("heading", { name: /non-linear factors/i })).toBeInTheDocument();
  expect(screen.getByText(/quadratic/i)).toBeInTheDocument(); // "quadratic" appears only in the Non-linear section
});
